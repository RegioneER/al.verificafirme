    ...
    public static partial class ExtensionMethods
    {
        public static MvcHtmlString AgidCheckBoxFor<TModel>(this HtmlHelper<TModel> helper, Expression<Func<TModel, bool>> expression, bool isDisabled = false, string id = null)
        {
            ...
            using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
            {
                ...
                writer.AddAttribute("aria-required", (unobtrusiveValitaionAttributes.ContainsKey("data-val-required")).ToString().ToLower());
        ...
        public static MvcHtmlString AgidToggle<TModel>(this HtmlHelper<TModel> helper, string name, string label, string id, bool? isChecked, bool isRequired = false, bool isDisabled = false)
        {
            /*
            <div class="form-check">
                <div class="toggles">
        ...
        public static MvcHtmlString AgidCheckBoxListFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression, MultiSelectList items, int maxItemsPerRow = 1, bool isRequired = false, bool isAgid2 = false)
        {
            /*
             <div>
                <div class="row">
        ...
        public static MvcHtmlString AgidCheckBoxList<TModel>(this HtmlHelper<TModel> helper, string name, string label, MultiSelectList items, int maxItemsPerRow = 1, bool isRequired = false, string id = null)
        {
            /*
             <div>
                <div class="row">
            ...
            using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
            {
                ...
                foreach (var item in items)
                {
                    ...
                    writer.AddAttribute(HtmlTextWriterAttribute.Type, "checkbox");
                    writer.AddAttribute(HtmlTextWriterAttribute.Name, name);
                    writer.AddAttribute(HtmlTextWriterAttribute.Id, $"{id ?? name}_{index + 1}");
                    if (item.Selected)
                        writer.AddAttribute(HtmlTextWriterAttribute.Checked, "checked");
                    writer.AddAttribute(HtmlTextWriterAttribute.Value, item.Value);
                    writer.RenderBeginTag(HtmlTextWriterTag.Input);
                ...
                if (isRequired)
                {
                    ...
                    writer.WriteLine("        if (visible_checkboxes.length > 0 && checked_checkboxes.length == 0)");
                    writer.WriteLine("        {");
                    writer.WriteLine($"            var error = $(\"<span></span>\").text(\"Il campo '{label}' è obbligatorio\");   ");
                    writer.WriteLine("            container.removeClass(\"field-validation-valid\").addClass(\"field-validation-error\");");
                    writer.WriteLine("            error.data(\"unobtrusiveContainer\", container);");
                    writer.WriteLine("            if (replace) {");
                    writer.WriteLine("                container.empty();");
