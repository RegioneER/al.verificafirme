...
namespace RER.Tools.MVC.Agid
{
    ...
    {
        public static MvcHtmlString AgidDropDownListFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression, SelectList items, string externalDivAdditionalClasses = null, string optionLabel = null, bool liveSearch = false, bool? required = null, string id = null, bool disabled = false, bool useBootstrapSelect = true, bool isAgid2 = false)
        {
            StringWriter stringWriter = new StringWriter();
            using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
            {
                if (isAgid2 && useBootstrapSelect)
                    useBootstrapSelect = false;
                ...
                //writer.AddAttribute(HtmlTextWriterAttribute.Class, $"bootstrap-select-wrapper{(disabled ? " disabled" : "")}{(!string.IsNullOrWhiteSpace(externalDivAdditionalClasses) ? " " + externalDivAdditionalClasses : "")}");
                writer.AddAttribute(HtmlTextWriterAttribute.Class, $"{(useBootstrapSelect ? "bootstrap-" : "")}select-wrapper{(disabled && useBootstrapSelect ? " disabled" : "")}{(!string.IsNullOrWhiteSpace(externalDivAdditionalClasses) ? " " + externalDivAdditionalClasses : "")}");
                writer.RenderBeginTag(HtmlTextWriterTag.Div);
        ...
        public static MvcHtmlString AgidMultipleDropDownListFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression, MultiSelectList items, string optionLabel = null, bool? required = null, string id = null, string externalDivAdditionalClasses = null, bool disabled = false, bool useBootstrapSelect = true, bool isAgid2 = false)
        {
            StringWriter stringWriter = new StringWriter();
            using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
            {
        ...
        public static MvcHtmlString AgidDropDownList<TModel>(this HtmlHelper<TModel> helper, string name, string label, SelectList items, object htmlAttributes, string optionLabel = null, bool liveSearch = false, string titleLabel = null, bool required = false, string id = null, int labelWidth = 12, int controlWidth = 12, bool disabled = false, string value = null, string externalDivAdditionalClasses = null, bool useBootstrapSelect = true, bool isAgid2= false)
        {
            /*
            ...
            using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
            {
                ...
                if (required)
                {
                    ...
                    //writer.AddAttribute(HtmlTextWriterAttribute.Class, $"col-{controlWidth} required");
                }
                else
                ...
                foreach (var item in items)
                {
                    if ((item.Selected && string.IsNullOrEmpty(value)) || value == item.Value)
                        writer.AddAttribute(HtmlTextWriterAttribute.Selected, "selected");
                    writer.AddAttribute(HtmlTextWriterAttribute.Value, item.Value);
                    writer.RenderBeginTag(HtmlTextWriterTag.Option);
                    writer.WriteEncodedText(item.Text);
